...
function parsePacksFromConfig(packsByLanguage, languages, configFile) {
    ...
    for (const [lang, packsArr] of Object.entries(packsByLanguage)) {
        ...
        for (const packStr of packsArr) {
            packs[lang].push(validatePackWithVersion(packStr, configFile));
        }
    }
    return packs;
}
...
function parsePacksFromInput(packsInput, languages) {
    ...
    return {
        [languages[0]]: packsInput.split(",").reduce((packs, pack) => {
            packs.push(validatePackWithVersion(pack, ""));
            return packs;
        }, []),
    };
}
 ...
 * @returns the original packStr
 */
function validatePackWithVersion(packStr, configFile) {
    ...
    const packEnd = Math.max(atIndex, colonIndex, packStr.length);
    const versionEnd = versionStart
        ? Math.max(packStr.indexOf(":"), packStr.length)
        : undefined;
    const pathEnd = pathStart ? packStr.length : undefined;
    const packName = packStr.slice(packStart, packEnd);
    const version = versionStart
        ? packStr.slice(versionStart, versionEnd)
        : undefined;
    const packPath = versionStart ? packStr.slice(pathStart, pathEnd) : undefined;
...
exports.validatePackWithVersion = validatePackWithVersion;
// exported for testing
